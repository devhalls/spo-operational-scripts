...
install_service() {
    if platform_ctl; then
        ...
        cp -p $dir/cardano-node.service $dir/$NETWORK_SERVICE.temp
        ...
        sudo cp -p $dir/$NETWORK_SERVICE.temp $SERVICE_PATH/$NETWORK_SERVICE
        sudo systemctl daemon-reload
        sudo systemctl enable $NETWORK_SERVICE
        rm $dir/$NETWORK_SERVICE.temp
        print 'INSTALL' "Node service created: $NETWORK_SERVICE" $green
    else
        print 'INSTALL' "Systemctl not supported, skipping and assuming docker install"
    fi
    return 0
}

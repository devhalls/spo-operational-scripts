...
tx_out() {
  amount=${1}
  totalBalance=${2}
  certificates=$(get_option --certificate-file "${@:3}")
  txIn=$(get_option --tx-in "${@:3}")
  paymentAddress=$(< $PAYMENT_ADDR)
  currentSlot=$(bash $(dirname "$0")/query.sh tip slot)
  outputPath=$NETWORK_PATH/temp
  witnessCount=0
  for arg in "$@"; do
    if [[ "$arg" == "--certificate-file" ]]; then ((witnessCount++)); fi
  done
...
tx_build() {
  ...
  txOutput=$(tx_out $amount $totalBalance $txIn $certificates)

  # Output.
  echo txRaw: $(get_param "$txOutput" "txRaw")
}
